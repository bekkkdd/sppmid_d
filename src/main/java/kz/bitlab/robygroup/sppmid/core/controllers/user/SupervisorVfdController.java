...
@Controller
@AllArgsConstructor
@RequestMapping(value = "/supervisorvfd")
public class SupervisorVfdController extends BaseController {
    ...
    public String readAdvanceAccount(Model model, @PathVariable(name = "advanceAccountId") Long advanceAccountId) {
        ...
        if(advanceAccount.getStage() < StaticConfig.ADVANCE_ACCOUNT_VFD_SUPERVISOR){
            return "redirect:/supervisorvfd/advanceaccounts";
        }
        ...
        return "user/supervisorvfd/readadvanceaccount";
    }
    ...
    public String signAdvanceAccount(Model model,
        ...
        if (advanceAccount != null
                && advanceAccount.getStage() == StaticConfig.ADVANCE_ACCOUNT_VFD_SUPERVISOR) {
            advanceAccount.setStage(StaticConfig.ADVANCE_ACCOUNT_VFD_SUPERVISOR_SIGNED);
            businessService.saveAdvanceAccount(advanceAccount);
        }
        return "redirect:/supervisorvfd/readadvanceaccount/" + advanceAccount.getId();
    }
    ...
    public String modifyAdvanceAccount(Model model,
        ...
        AdvanceAccount advanceAccount = businessService.getAdvanceAccountById(id);
        if (advanceAccount != null && advanceAccount.getStage() == StaticConfig.ADVANCE_ACCOUNT_VFD_SUPERVISOR) {
    ...
    public String businessTrips(Model model, @RequestParam(name = "page", defaultValue = "1") int page) {
        ...
        model.addAttribute("user", user);
        model.addAttribute("advanceAccounts", businessService.getAdvanceAccountsByStageGreaterThanEqual(StaticConfig.ADVANCE_ACCOUNT_VFD_SUPERVISOR));

        return "user/supervisorvfd/advanceaccounts";
    }
}
