...
@Controller
@AllArgsConstructor
@RequestMapping(value = "/supervisorgo")
public class SupervisorGOController extends BaseController {
    ...
    @PreAuthorize("hasAnyRole('ROLE_RUKOVODITEL_GO')")
    public String processList(Model model, @RequestParam(name = "page", defaultValue = "1") int page){
        ...
        return "user/supervisorgo/processlist";
    ...
    public String readProcess(Model model, @CookieValue(name = "currencyId", defaultValue = "1") Long currencyId, @PathVariable(name = "id") Long id){
        ...
        if(process.getStage()==StaticConfig.FOURTH_STAGE&&process.getStatus()==StaticConfig.STATUS_SIGNED){

            process.setStage(StaticConfig.FIFTH_STAGE);
            process.setStageName(StaticConfig.processStageNames.get(StaticConfig.FIFTH_STAGE));
            process.setStageRole(StaticConfig.processStageRoles.get(StaticConfig.FIFTH_STAGE));
        ...
        }else if(process.getStage()==StaticConfig.SIXTH_STAGE&&process.getStatus()==StaticConfig.STATUS_SIGNED){
        ...
        List<Users> zayavitels = userService.getZayavitelsByDepartment(currentUser.getDepartment());
        model.addAttribute("zayavitels", zayavitels);

        return "user/supervisorgo/processdetails";
    }
    ...
    public String businessTrips(Model model, @RequestParam(name = "page", defaultValue = "1") int page) {
        ...
        List<BusinessTrips> businessTrips = businessService.getBusinessTripsByStageGreaterThanEqual(StaticConfig.BUSINESS_TRIP_GO_SUPERVISOR);

        model.addAttribute("user", user);
        model.addAttribute("pageCount", pageCountFinal);
        model.addAttribute("businessTrips", businessTrips);

        return "user/supervisorgo/businesstrips";
    }
    ...
    public String readBusinessTrip(Model model, RedirectAttributes redirectAttributes, @PathVariable(name = "businessTripId") Long businessTripId) {
        ...
        if(businessTrip == null || businessTrip.getStage() < StaticConfig.BUSINESS_TRIP_GO_SUPERVISOR)
        {
            redirectAttributes.addFlashAttribute("error", "SIGNED");
            return "redirect:/supervisorgo/businesstrips";
        }
        ...
        return "user/supervisorgo/readbusinesstrip";
    }
    ...
    public String signBusinessTrip(Model model,
        ...
        if (businessTrip != null && businessTrip.getStage() == StaticConfig.BUSINESS_TRIP_GO_SUPERVISOR) {
            businessTrip.setStage(StaticConfig.BUSINESS_TRIP_GO_SECRETARY);
            businessService.saveBusinessTrip(businessTrip);
            return "redirect:/supervisorgo/readbusinesstrip/" + businessTrip.getId();
        }
        return "redirect:/supervisorgo/businesstrips";
    }
    ...
    public String executeSign(
        ...
        if(process.getStage()==StaticConfig.FIFTH_STAGE&&process.getStatus()==StaticConfig.STATUS_UNDER_CONSIDERATION){
            if(action.equals("sign")){
                ...
                for(Users zvt : zayavitels){
                    if(request.getParameter("zayavitel_"+zvt.getId())!=null) {
                        performers.add(zvt);
                    }
                }

                process.setGoPerformers(performers);
        ...
        }else if(process.getStage()==StaticConfig.SEVENTH_STAGE&&process.getStatus()==StaticConfig.STATUS_UNDER_CONSIDERATION){
        ...
        return "redirect:/supervisorgo/processdetails/"+id+"?success";
    }
    ...
    public String addSectionData(Model model,
        ...
        return "redirect:/supervisorgo/processdetails/"+id;
    ...
    public String addSectionData(Model model,
        ...
        return "redirect:/supervisorgo/processdetails/"+processId;
    ...
    public String combineProcesses(HttpServletRequest request) {
        ...
        return "redirect:/supervisorgo/processlist";
    }
    ...
    public String passports(Model model, @RequestParam(name = "page", defaultValue = "1") int page) {
        ...
        List<PassportRequest> passportRequests = businessService.getPassportRequestsByStageGreaterThanEqual(StaticConfig.PASSPORT_REQUEST_GO_SUPERVISOR);

        model.addAttribute("user", user);
        model.addAttribute("pageCount", pageCountFinal);
        model.addAttribute("passportRequests", passportRequests);

        return "user/supervisorgo/passportrequests";
    }
    ...
    public String readPassportRequest(Model model, @PathVariable(name = "passportRequestId") Long passportRequestId) {
        ...
        return "user/supervisorgo/readpassportrequest";
    }
    ...
    public String signPassportRequest(Model model,
        ...
        if (passportRequest != null && passportRequest.getStage() == StaticConfig.PASSPORT_REQUEST_GO_SUPERVISOR) {
            passportRequest.setStage(StaticConfig.PASSPORT_REQUEST_GO_SECRETARY);
            businessService.savePassportRequest(passportRequest);
            return "redirect:/supervisorgo/readpassportrequest/" + passportRequest.getId();
        }
        return "redirect:/supervisorgo/passportrequests";
    ...
    public String modifyPassportRequest(Model model,
        ...
        return "redirect:/supervisorgo/passports";
    }
}
